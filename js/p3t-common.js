// p3t-common.js — Page3/Page4 共享：时间控制（P3/P4 共享 sessionStorage）+ 表格辅助 + 五行单元格渲染
...
function yoyStr(c, l) {
    if (!(l > 0)) return '<span class="p3t-ny">--</span>';
    var v = (c - l) / l * 100;
    return v >= 0 ? '<span class="p3t-yu">+' + v.toFixed(0) + '%</span>' : '<span class="p3t-yd">' + v.toFixed(0) + '%</span>';
}

// 五行单元格（Tar/Act/Act%/LY/YOY）：未来月（>所选月）仅 Act/Act%/YOY 留空，Tar/LY 显示全月
function p3tCell(mt, i, m, t, a, l) {
    var c = '--';
    if (mt === 'Tar') c = t > 0 ? t : '--';
    ...
    return '<td' + (i === m ? ' class="p3t-cur"' : '') + '>' + c + '</td>';
}
function p3tYtdCell(mt, yt, ya, yl) {
    var yc = '--';
    ...
    return '<td class="p3t-ytd">' + yc + '</td>';
}
// 五行整行：calcMonth(i) 返回 {t,a,l}；yt/ya/yl 为该行累计（传入初始 0）
function p3tMetricRowHTML(mt, m, calcMonth, yt, ya, yl) {
    var h = '<tr class="p3t-mtr"><td class="p3t-lbl">' + mt + '</td>';
    for (var i = 1; i <= 12; i++) {
        var v = calcMonth(i);
        if (i <= m) { yt += v.t; ya += v.a; yl += v.l; }
        h += p3tCell(mt, i, m, v.t, v.a, v.l);
    }
    return { h: h + p3tYtdCell(mt, yt, ya, yl) + '</tr>', yt: yt, ya: ya, yl: yl };
}

// 时间控制：构建年月选项、读共享 sessionStorage（新开网页默认当前年月）、onchange 保存并回调
function initP3tTime(selY, selM, yearKeys, onchange) {
    var yArr = Object.keys(yearKeys).sort();
    var now = new Date();
    var defY = String(now.getFullYear()), defM = now.getMonth() + 1;
    try {
        var _sh = JSON.parse(sessionStorage.getItem('p3t_time') || 'null');
    ...
    function save() { try { sessionStorage.setItem('p3t_time', JSON.stringify({ y: $(selY).val(), m: parseInt($(selM).val(), 10) })); } catch (e) {} }
